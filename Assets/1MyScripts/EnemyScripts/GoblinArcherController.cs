...
public class GoblinArcherController : MonoBehaviour {
		...
		if (!stunned) 
		{
			  RaycastHit2D hit;
				...
				if (chasing) 
				{
					...
					if (!attacking && Vector2.Distance(transform.position, player.transform.position) > bowAttackRange) 
					{
						transform.position = Vector2.MoveTowards(transform.position, player.transform.position, chaseSpeed * Time.deltaTime);						
					}

					if (Vector2.Distance(transform.position, player.transform.position) < bowAttackRange && attackTimer <= 0) 
					{
						attacking = true;
	...
	void FixedUpdate () {
		...
		} else 
		{
			...
			if (!attacking) 
			{
				if (chasing && (Vector2.Distance(transform.position, player.transform.position) > bowAttackRange) && anim.GetInteger("AnimState") != 1) 
				{
					anim.SetInteger("AnimState", 1);
				} else if (chasing && (Vector2.Distance(transform.position, player.transform.position) <= bowAttackRange)) 
				{
					anim.SetInteger("AnimState", 0);
				}
			}
