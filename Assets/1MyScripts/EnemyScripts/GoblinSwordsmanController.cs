...
public class GoblinSwordsmanController : MonoBehaviour {
	...
	void FixedUpdate () {
		...
		} else 
		{
			...
			if (chasing && (Vector2.Distance(transform.position, player.transform.position) > meleeAttackRange) && anim.GetInteger("AnimState") != 1) 
			{
				anim.SetInteger("AnimState", 1);
			} else if (chasing && (Vector2.Distance(transform.position, player.transform.position) <= meleeAttackRange)) 
			{
			...
			{
				bool playerLeft = playerToLeft();
				// Spin attack can hit player when they move behind
				if ((facingLeft && !playerLeft) || (!facingLeft && playerLeft) && anim.GetInteger("AnimState") != 2) 
				{
				...
				//	Normal attacked
				} else if (anim.GetInteger("AnimState") != 3)
				{
